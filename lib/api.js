...
export async function login(email, password) {
  // 1. Cihaz ID'sini e-postaya özel ve KALICI hale getiriyoruz (Çoğalmayı önler)
  const userKey = `voia_device_${email}`;
  let installationId = localStorage.getItem(userKey);

  if (!installationId) {
    installationId = typeof crypto !== 'undefined' && crypto.randomUUID 
      ? crypto.randomUUID() 
      : 'id-' + Date.now();
    localStorage.setItem(userKey, installationId);
  }

  // 2. Panel'in okuyabilmesi için standart anahtara kopyalıyoruz (409 hatasını önler)
...
export async function logout() {
  if (typeof window !== 'undefined') {
    // 3. Çıkışta SADECE standart anahtarı siliyoruz. 
    // E-postaya özel anahtar silinmediği için tekrar girişte aynı ID kullanılır ve klonlanma olmaz.
...
/**
 * Kullanıcının bildirim ve arama geçmişini getirir.
 * GET /api/reminder-history
 * Opsiyonel filtreleme: ?reminderId=...
 */
export async function getReminderHistory(reminderId = null) {
  const url = reminderId 
    ? `/api/reminder-history?reminderId=${reminderId}` 
    : "/api/reminder-history";
    
  try {
    // Önce gerçek backend'i deniyoruz
    return await requestWithRefresh(url);
  } catch (error) {
    // Eğer backend henüz hazır değilse (Cannot GET hatası vb. verirse) sayfayı patlatmak yerine sahte veri dönüyoruz.
    console.warn("SİSTEM UYARISI: Backend /api/reminder-history henüz hazır değil. Tasarım için sahte veri (mock) gösteriliyor.");
    
    // Şu anki zamana göre dinamik tarihler oluşturalım ki BUGÜN ve DÜN grupları çalışsın
    const now = new Date();
    ...
    return [
      {
        historyId: "mock-1",
        reminder: { title: "İlaç Hatırlatıcısı" },
      ...
      {
        historyId: "mock-2",
        reminder: { title: "Toplantı" },
        historyType: "PUSH",
        status: "FAILED",
        provider: "FCM",
        sentAt: twoHoursAgo.toISOString(),
        errorMessage: "Kullanıcı cihazında bildirim izinleri kapalı."
      },
      {
        historyId: "mock-3",
        reminder: { title: "Su İçme Hatırlatıcısı" },
        historyType: "VOICE_CALL",
        status: "FAILED",
        provider: "Twilio",
        sentAt: yesterday.toISOString(),
        errorMessage: "Kullanıcı meşgule attı."
      },
      {
        historyId: "mock-4",
        reminder: { title: "Kredi Kartı Son Ödeme" },
        historyType: "PUSH",
        status: "SUCCESS",
        provider: "FCM",
        sentAt: yesterday.toISOString(),
...
export async function getSilentHours() {
  try {
    return await requestWithRefresh("/api/silent-hours");
  } catch (error) {
    console.warn("SİSTEM UYARISI: Backend /api/silent-hours henüz hazır değil. Sayfanın çökmemesi için boş dizi dönülüyor.");
    // Backend hazır olana kadar hata patlatmak yerine boş dizi dönüyoruz.
    // UI zaten bu boş diziyi görünce tüm günleri "Kapalı" ve varsayılan saatlerle başlatacak.
    return []; 
  }
}
