...
export const updateUserProfile = async (profileData) => {
  ...
  const data = await response.json();
  if (!response.ok) {
    throw new Error(data.message || 'Profil bilgileri güncellenemedi.');
  }
  return data;
};
...
export const getDevices = async () => {
  ...
  const data = await response.json();
  if (!response.ok) {
    throw new Error(data.message || 'Cihazlar alınamadı.');
  }
  return data; // Dizi (array) olarak cihaz listesini döner
};